...
- loads the JSON metadata produced by `md-gen` from `source/temp/metadata`
- separates verified sequences from loose page scans
- scores candidate page links using a local LLM scorer
- writes a deterministic merge plan JSON file into `source/merge-plan.json`
...
- `plan` supports `--edge-scorer llm`
- `plan` supports `--rolling-window`, `--llm-endpoint-url`, `--llm-model-name`, `--llm-timeout-seconds`, `--llm-max-retries`, and `--score-prompt`
- `plan` reads metadata from `source/temp/metadata` and writes `source/merge-plan.json`
- `apply` requires `--source` and reads `source/merge-plan.json`
